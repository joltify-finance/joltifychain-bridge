...
func (s *FileStateMgrTestSuite) TestSaveOutBoundState(c *C) {
	...
	err := fsm.SaveOutBoundState(testReqs[:99])
	c.Assert(err, IsNil)
	...
	// now we add another tx
	fsm.SaveOutBoundState([]*common.OutBoundReq{testReqs[99]})

	loaded2, err := fsm.LoadOutBoundState()
	c.Assert(err, IsNil)
	c.Assert(loaded2[99].TxID, Equals, testReqs[99].TxID)
}
